/**
 * JSON Schema for the structured report returned to the browser.
 *
 ...
 *
 * The whole report is deliberately NOT requested in one call. The API compiles a
 * schema into a grammar and rejects one that gets too large, and the full report
 * is well over that ceiling — adding `viewsAnalyzed` alone was enough to trip it.
 * So the fields are split into two calls (see ASSESSMENT_SCHEMA and PLAN_SCHEMA
 * below), each of which measured comfortable headroom, and the two results are
 * merged. The split is also useful on its own terms: the plan is written knowing
...
/** Call 1 of the report pass: what was seen, and what it might be. */
export const ASSESSMENT_KEYS = ['emergency', 'viewsAnalyzed', 'videoQuality', 'gaitAssessment', 'differential'];

/** Call 2: what to do about it, written with the differential already in hand. */
export const PLAN_KEYS = ['strideOptimizationPlan', 'vetVisitChecklist', 'limitations'];

export const ASSESSMENT_SCHEMA = schemaOf(ASSESSMENT_KEYS);
export const PLAN_SCHEMA = schemaOf(PLAN_KEYS);

/** The merged shape the frontend consumes. Never sent as a single grammar. */
export const REPORT_SCHEMA = schemaOf([...ASSESSMENT_KEYS, ...PLAN_KEYS]);
